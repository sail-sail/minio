...
import {
  Http,
  Https,
  StreamStream as Stream,
  BlockStream2,
  querystring,
  promisify,
} from '../../deps.ts'
import { xml as Xml } from '../xml/xml.js'
// import async from 'async'
// import mkdirp from 'mkdirp'
// import path from 'path'
// import _ from 'lodash'
import { Builder as xml2js_Builder } from "../xml/xml2js_builder.js"
...
const _ = {
  ...
  pick(object, paths) {
    ...
    for (const path of paths) {
      if (object[path]) result[path] = object[path];
    }
    return result;
  },
  ...
  forEach(obj, iteratee) {
    return Object.keys(obj).forEach((key) => iteratee(obj[key], key, obj))
  }
};
...
import {
  ...
  makeDateLong, getVersionId, sanitizeETag,
  toMd5, toSha256,
...
import { signV4, presignSignatureV4, postPresignSignatureV4 } from './signing.ts'
...
import { getS3Endpoint } from './s3_endpoints.ts'
...
export class Client {
  ...
  logHTTP(reqOptions, response, err) {
    ...
    var logHeaders = (headers) => {
      _.forEach(headers, (v, k) => {
        ...
        // }
        this.logStream.write(new TextEncoder().encode(`${k}: ${v}\n`))
      })
      this.logStream.write(new TextEncoder().encode('\n'))
    }
    this.logStream.write(new TextEncoder().encode(`REQUEST: ${reqOptions.method} ${reqOptions.path}\n`))
    logHeaders(reqOptions.headers)
    if (response) {
      this.logStream.write(new TextEncoder().encode(`RESPONSE: ${response.statusCode}\n`))
      logHeaders(response.headers)
    }
    if (err) {
      this.logStream.write(new TextEncoder().encode('ERROR BODY:\n'))
      var errJSON = JSON.stringify(err, null, '\t')
      this.logStream.write(new TextEncoder().encode(`${errJSON}\n`))
    }
  }
  ...
  makeRequestStream(options, stream, sha256sum, statusCodes, region, returnResponse, cb) {
    ...
    var _makeRequest = (e, region) => {
      ...
      if (!this.anonymous) {
        ...
        // let date = new Date("2022-05-24 08:39:54")// TODO Sail
        let date = new Date()
  ...
  getPartialObject(bucketName, objectName, offset, length, getOpts={}, cb) {
    ...
    this.makeRequest({method, bucketName, objectName, headers, query}, '', expectedStatusCodes, region, true, cb)
  }
  ...
  removeObjects(bucketName, objectsList, cb) {
    ...
    for (let i = 0; i < result.listOfList.length; i++) {
      const list = result.listOfList[i];
      var objects=[]
      ...
      const builder = new xml2js_Builder({ headless: true })
  ...
  setBucketVersioning(bucketName,versionConfig, cb) {
    ...
    var method = 'PUT'
    var query = "versioning"
    var builder = new xml2js_Builder({rootName:'VersioningConfiguration', renderOpts:{'pretty':false}, headless:true})
  ...
  setTagging(taggingParams){
    ...
    const encoder = new TextEncoder()
    const headers ={}
    const builder = new xml2js_Builder({ headless:true,renderOpts:{'pretty':false},})
  ...
  applyBucketLifecycle(bucketName, policyConfig, cb){
    ...
    const encoder = new TextEncoder()
    const headers ={}
    const builder = new xml2js_Builder({ rootName:'LifecycleConfiguration', headless:true, renderOpts:{'pretty':false},})
  ...
  setObjectLockConfig(bucketName, lockConfigOpts={}, cb) {
    ...
    const builder = new xml2js_Builder({rootName:'ObjectLockConfiguration', renderOpts:{'pretty':false}, headless:true})
  ...
  putObjectRetention(bucketName, objectName, retentionOpts={}, cb) {
    ...
    const builder = new xml2js_Builder({rootName:'Retention', renderOpts:{'pretty':false}, headless:true})
    const params ={}
  ...
  setBucketEncryption(bucketName, encryptionConfig, cb) {
    ...
    let builder = new xml2js_Builder({rootName:'ServerSideEncryptionConfiguration', renderOpts:{'pretty':false}, headless:true})
  ...
  setBucketReplication(bucketName, replicationConfig={}, cb) {
    ...
    const builder =  new xml2js_Builder({ renderOpts:{'pretty':false},headless: true })
  ...
  setObjectLegalHold(bucketName, objectName, setOpts={}, cb){
    ...
    const builder = new xml2js_Builder({rootName:'LegalHold', renderOpts:{'pretty':false}, headless:true})
    const payload = builder.buildObject(config)
    const headers = {}
    headers['Content-MD5'] = toMd5(payload)
    ...
    return  Promise.all(sourceObjStats).then(srcObjectInfos =>{
      ...
      const performUploadParts = async (uploadId) =>{
        ...
        for (let i = 0; i < uploadList.length; i++) {
          const partConfig = uploadList[i];
          await new Promist((resolve, _reject) => {
            me.uploadPartCopy(partConfig, (err, res) => {
              if (err) {
                resolve(this.abortMultipartUpload(destObjConfig.Bucket, destObjConfig.Object, uploadId, cb));
                return;
              }
              const partsDone = res.map((partCopy)=>({etag:partCopy.etag, part:partCopy.part}))
              me.completeMultipartUpload(destObjConfig.Bucket, destObjConfig.Object, uploadId, partsDone, cb)
              resolve(undefined)
            });
          });
        }
  ...
  selectObjectContent(bucketName, objectName, selectOpts={}, cb) {
    ...
    const builder = new xml2js_Builder({rootName:'SelectObjectContentRequest', renderOpts:{'pretty':false}, headless:true})
    const payload = builder.buildObject(config)
