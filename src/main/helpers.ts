// deno-lint-ignore-file no-explicit-any no-var no-inner-declarations prefer-const ban-types
/*
...
import * as fxp from "../fast-xml-parser/src/parser.js";
...
export function getScope(region: any, date: Date, serviceName="s3") {
...
export function isVirtualHostStyle(endpoint: string, protocol: string, bucket: string|string[], pathStyle: any) {
...
export function probeContentType(path: string) {
  ...
  let contentType = mimeExtMapping[ext][0];
  if (!contentType) {
...
export function isReadableStream(arg: any) {
...
export function isValidDate(arg: any) {
...
export function pipesetup(...streams: any[]) {
  return streams.reduce((src, dst) => {
    src.on('error', (err: Error) => dst.emit('error', err))
    return src.pipe(dst)
  })
}
...
export function insertContentType(metaData: any, filePath: string) {
...
export function prependXAMZMeta(metaData: any) {
  var newMetadata = Object.assign({}, metaData)
  for (var key in metaData) {
...
export function extractMetadata(metaData: { [x: string]: any; }) {
  var newMetadata: any = {}
  for (var key in metaData) {
...
const objectToBuffer = (payload: string) =>{
...
export const toMd5=(payload: string)=>{
  const payLoadBuf = objectToBuffer(payload)
  // use string from browser and buffer from nodejs
  // browser support is tested only against minio server
  return createHash('md5').update(payLoadBuf).digest().toString('base64') as string;
}

export const toSha256=(payload: string)=>{
  return createHash('sha256').update(payload).digest('hex') as string;
}
...
export const sanitizeObjectKey=(objectName: any)=>{
 ...
 */
function getEncryptionHeaders (encConfig: { type: any; SSEAlgorithm: any; KMSMasterKeyID: any; }) {
  const encType = encConfig.type
  const encHeaders:any = {}
...
export class CopySourceOptions {
  ...
  constructor({
    ...
    this.MatchETag=NoMatchETag
    this.MatchModifiedSince=MatchModifiedSince
    this.MatchModifiedSince=MatchUnmodifiedSince
...
let startIndexParts = []
let endIndexParts = []
export function calculateEvenSplits(size: number , objInfo: { Start: any; })  {
  if (size === 0) {
    return null
  }
